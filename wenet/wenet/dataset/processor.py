...
if platform.system() != 'Windows':
    try:
        ...
        cpu_info_grep = [l for l in cpu_info.split('\n') if 'Vendor ID' in l]
        cpu_info = cpu_info_grep[0] if cpu_info_grep else ""
        # 0x48 --> HiSilicon
        ...
        pass  # Non-Linux platform, skip Kunpeng detection
...
def decode_wav(sample):
    ...
    if 'start' in sample:
        ...
        try:
            ...
            import soundfile
            info = soundfile.info(wav_file) if isinstance(wav_file, str) else soundfile.SoundFile(wav_file)
        ...
        try:
            ...
            import soundfile
            data, _sr = soundfile.read(wav_file, start=start_frame, stop=end_frame)
            data = data.T if data.ndim > 1 else data.reshape(1, -1)
