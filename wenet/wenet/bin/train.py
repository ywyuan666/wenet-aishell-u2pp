...
def main():
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        try:
            ...
            logging.warning('CV returned empty results, skipping.')
    ...
    if final_epoch is not None and rank == 0:
        ...
        try:
            ...
            # symlink not supported on some Windows systems, copy instead
            import shutil
            src = os.path.join(args.model_dir, '{}.pt'.format(final_epoch))
            if os.path.exists(src):
                shutil.copy2(src, final_model_path)
        writer.close()
    if dist.is_initialized():
        dist.barrier(
