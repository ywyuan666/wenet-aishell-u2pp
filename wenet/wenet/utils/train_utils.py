...
try:
    ...
    estimate_zero2_model_states_mem_needs_all_cold = None
...
def init_distributed(args):
    ...
    if args.train_engine in ["torch_ddp", "torch_fsdp"]:
        ...
        elif "cpu" in args.device or not torch.cuda.is_available():
            logging.info('Running on CPU. Skipping GPU device setup.')
        else:
            logging.error("not supported device: {}".format(args.device))
        if dist.is_available() and world_size > 1:
            dist.init_process_group(args.dist_backend)
    elif args.train_engine == "deepspeed" and deepspeed is not None:
...
def wrap_cuda_model(args, model, configs=None):
    ...
    if args.train_engine == "torch_ddp":  # native pytorch ddp
        ...
        if dist.is_initialized():
            ...
            logging.info('Single-process mode (DDP not initialized), skipping DDP wrapper.')
            model.train()
