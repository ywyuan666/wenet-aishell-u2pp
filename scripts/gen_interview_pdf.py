#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 WeNet/AISHELL/U2++ 技术总结笔记 PDF"""
...
# 一、项目概览与自我介绍
# ════════════════════════════════════
story.append(Paragraph("一、项目概览与自我介绍", s_h1))
story.append(divider())

story.append(Paragraph("1.1  一句话介绍（30 秒电梯演讲）", s_h2))
...
story.append(Paragraph("1.2  一分钟介绍", s_h2))
...
story.append(Paragraph("1.3  三分钟完整介绍（完整介绍版本）", s_h2))
story.append(Paragraph(
    "技术评审您好，我开发的这个项目是基于 <b>WeNet 框架</b>在 <b>AISHELL-1 中文预测数据集</b>上实现 <b>U2++ 架构</b>的语音识别系统。"
    "我分四个层面来介绍：", s_body))
...
# 三、核心知识问答 & 问答解析
# ════════════════════════════════════
story.append(Paragraph("三、核心知识问答 & 问答解析", s_h1))
story.append(divider())
...
qa_basic = [
    ("什么是端到端语音识别？与传统混合系统相比有什么优势？",
     ...
     "技术评审可能在等你的辩证分析——既要说优势也要谈缺点，体现你的工程思维。"),
...
qa_open = [
    ...
    ("你的项目中哪些部分最有成就感？（技术深挖中的自我评价问题）",
     ...
     "技术评审，我觉得好的项目不仅仅是模型精度高，更要有完整的工程化闭环。"),
...
# 四、技术评审深挖方向
# ════════════════════════════════════
story.append(Paragraph("四、技术评审深挖方向", s_h1))
story.append(divider())
...
# 五、技术提示与策略
# ════════════════════════════════════
story.append(Paragraph("五、技术提示与策略", s_h1))
story.append(divider())

story.append(Paragraph("5.1  技术面通用策略", s_h2))
story.append(Paragraph(
    "<b>核心原则：把技术评审当同事，展示你的思考过程而非背诵答案。</b>", s_body))
story.append(Spacer(1, 4))
story.append(bullet("<b>结构化表达</b>: 面对任何问题，用 '第一、第二、第三' 或 '首先、其次、最后' 的方式组织回答，清晰有条理"))
story.append(bullet("<b>STAR 法则</b>: 介绍项目时按 Situation→Task→Action→Result 展开，量化结果（如 CER 4.61%，RTF 0.008）"))
story.append(bullet("<b>控制深度</b>: 技术评审问 '什么是 CTC' 时，不要只背定义。先简单解释（1 句），然后主动问 '需要我深入讲一下前向后向算法吗？'"))
story.append(bullet("<b>暴露思考</b>: 遇到不会的问题直接说 '这个我不太确定，但我推测可能是...' 或 '我可以用类比的方式理解...' 展现逻辑推理能力"))
story.append(bullet("<b>工程思维</b>: 每个算法问题至少顺带提一句工程实践的考虑（实现细节、性能、边界情况、兼容性等）"))
story.append(Spacer(1, 6))

story.append(Paragraph("5.2  项目介绍的 STAR 法则（演示开场必备）", s_h2))
story.append(highlight_box([
    Paragraph("<b>Situation（背景）</b>: 语音识别在工业应用中有两个核心场景——实时流式交互（语音助手、客服质检）和离线高质量转写（会议记录、字幕生成）。传统方案需要分别部署两套模型，维护成本高。", s_body),
    Paragraph("<b>Task（任务）</b>: 我需要在 AISHELL-1 中文数据集上构建一套统一流式和非流式的端到端 ASR 系统，达到业界竞争力水平（CER ~5%），同时具备完整的工程部署能力。", s_body),
    Paragraph("<b>Action（行动）</b>: ① 基于 WeNet 框架，采用 Conformer U2++ 架构，构建 12 层 Conformer Encoder + CTC + 6 层 Bi-Transformer Decoder；② 实现 Dynamic Chunk Training 支持可变延迟解码；③ 搭建完整的 CI/测试/可视化和部署链路。", s_body),
    Paragraph("<b>Result（结果）</b>: Attention Rescoring 达到 CER 4.61%，CTC Greedy 达到 CER 5.21%（chunk=16），RTF 0.008。项目包含 Gradio Web UI、HuggingFace 模型格式、4 张 Benchmark 可视化图表、CI 自动化测试等完整工程化能力。", s_body),
]))
story.append(Spacer(1, 6))

story.append(Paragraph("5.3  常见陷阱与应对", s_h2))
story.append(bullet("<b>陷阱：技术评审问结果为什么不是最优的</b> → 回应：'我们关注的是在有限计算资源下取得有竞争力的效果，同时工程完整度更高。AISHELL-1 目前纯 E2E 的最好结果是 4.3-4.5%（需要更长的训练），我们的 4.61% 在无外部 LM 的情况下已经很不错。'"))
story.append(bullet("<b>陷阱：技术评审说这个项目看起来像 '调参工程'</b> → 回应：'调参只是工作的一部分。我投入更多时间在：理解 Conformer 中卷积和注意力的互补机制（在论文中给出了自己的理解）、构建完整的 CI/CD 和 Web UI 部署链路、实现 HuggingFace 格式转换的模型兼容层。调参是手段，工程能力是目标。'"))
story.append(bullet("<b>陷阱：技术评审问你不懂的地方</b> → 回应：'这块我了解得不深，但我知道它涉及的核心概念是 XXX。如果我有更多时间，我会通过阅读原论文和源码来补充这方面的理解。' 记住技术评审更看重学习能力而非已有知识存量。"))
story.append(bullet("<b>陷阱：被问及数据隐私</b> → 回应：'项目中全部使用的是公开数据集 AISHELL-1。如果涉及敏感数据（如医疗记录），需要做数据脱敏，并在模型层面做差分隐私训练。我们从工程上已经预留了数据过滤和隐私保护的接口。'"))
story.append(Spacer(1, 6))

story.append(Paragraph("5.4  反问环节建议（向技术评审提问）", s_h2))
story.append(Paragraph(
    "当技术评审问 '你还有什么想问我的？' 时，以下问题能体现你的深度和关注力：", s_body))
story.append(bullet("'贵团队的 ASR 系统目前是自研的还是基于开源框架？在 WeNet/K2/Whisper 的生态中怎么选择的？'"))
story.append(bullet("'我对流式 ASR 的延迟优化很感兴趣，你们在线上做的延迟大概是 P99 多少？量化了吗？'"))
story.append(bullet("'有没有尝试过 Conformer 之外的架构？Squeezeformer 或 Branchformer 你们怎么看？'"))
story.append(bullet("'团队里 ASR 和 NLP/LLM 的配合多吗？目前有没有尝试用 LLM 做 ASR 后处理纠错？'"))
story.append(bullet("'如果我入职，前三个月的重点工作方向是什么？是模型改进还是工程优化？'"))
story.append(PageBreak())

# ════════════════════════════════════
# 六、技术深挖进阶准备
# ════════════════════════════════════
story.append(Paragraph("六、技术深挖进阶准备", s_h1))
story.append(divider())

story.append(Paragraph("6.1  从零推导 Conformer", s_h2))
story.append(Paragraph(
    "技术考核中可能会要求你在白板上画出 Conformer 块的完整结构。掌握以下推导：", s_body))
...
story.append(Paragraph("6.2  CTC 对齐原理", s_h2))
story.append(Paragraph(
    "CTC 的对齐算法（前向后向）是核心考核点。核心掌握：", s_body))
...
story.append(Paragraph("6.3  RNN-T vs Attention vs CTC", s_h2))
story.append(Table([
...
story.append(Paragraph("6.4  流式解码的工程实现", s_h2))
story.append(Paragraph(
...
story.append(Paragraph("6.5  模型量化与加速（加分项）", s_h2))
...
# 七、技术交流注意事项
# ════════════════════════════════════
story.append(Paragraph("七、技术交流注意事项", s_h1))
story.append(divider())

story.append(Paragraph("7.1  字节跳动（抖音语音组）", s_h2))
story.append(bullet("重点关注 <b>端到端 ASR 的工业落地</b>，特别是流式交互的延迟优化"))
story.append(bullet("可能问: '如果抖音直播间的 ASR 有 500ms 的延迟，你怎么优化？'"))
story.append(bullet("技术栈: WeNet / K2 / 自研引擎 + TensorRT 部署"))
story.append(bullet("技术风格: 工程导向，会深挖部署细节"))
story.append(Spacer(1, 4))

story.append(Paragraph("7.2  百度（语音技术部）", s_h2))
story.append(bullet("中国语音技术的老牌团队，自研 SMLTA（流式多级 Transformer）架构"))
story.append(bullet("可能问: 'U2++ 和我们百度的 SMLTA 有什么异同？'"))
story.append(bullet("技术栈: PaddlePaddle + 自研模型"))
story.append(bullet("技术风格: 理论深度 + 论文阅读能力并重"))
story.append(Spacer(1, 4))

story.append(Paragraph("7.3  阿里巴巴（达摩院语音实验室）", s_h2))
story.append(bullet("聚焦 <b>多模态</b> 和 <b>大规模预训练</b>（Spoken Language Understanding）"))
story.append(bullet("可能问: 'ASR 和 NLP 的结合你认为怎么做？语音助手的产品形态会怎么演进？'"))
story.append(bullet("技术栈: TF / PyTorch + 自研 + 阿里云平台"))
story.append(bullet("技术风格: 既看工程也看研究视野"))
story.append(Spacer(1, 4))

story.append(Paragraph("7.4  腾讯（AI Lab / 微信智聆）", s_h2))
story.append(bullet("微信语音识别、腾讯会议转录等产品，关注 <b>低资源场景</b> 和 <b>领域自适应</b>"))
story.append(bullet("可能问: '假如微信语音转文字在方言上效果很差，你怎么解决？'"))
story.append(bullet("技术栈: PyTorch + 自研推理引擎"))
story.append(bullet("技术风格: 产品思维 + 技术深度"))
story.append(Spacer(1, 4))

story.append(Paragraph("7.5  华为（2012 实验室）", s_h2))
story.append(bullet("关注 <b>端侧部署</b> 和 <b>异构计算</b>（昇腾芯片适配）"))
story.append(bullet("可能问: '怎么把 50M 的 Conformer 模型部署到手机或 NPU 上？'"))
story.append(bullet("技术栈: MindSpore + DaVinci 芯片"))
story.append(bullet("技术风格: 系统思维 + 硬件理解"))
story.append(PageBreak())

# ════════════════════════════════════
# 八、推荐阅读与资料
# ════════════════════════════════════
story.append(Paragraph("八、推荐阅读与资料", s_h1))
story.append(divider())

story.append(Paragraph("8.1  必读论文（按优先级排序）", s_h2))
...
story.append(Paragraph("8.2  进阶阅读", s_h2))
...
story.append(Paragraph("8.3  技术博客与资源", s_h2))
